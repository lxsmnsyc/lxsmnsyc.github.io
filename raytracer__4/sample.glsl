...
vec3 getSurfaceColor(Sphere s, vec3 pos){
    ...
    float longi = atan(diff.y/SPHERE_RADIUS);
    float lati = acos(diff.z/SPHERE_RADIUS);

    longi = (longi + PI)*RAD2DEG;
    lati = (lati + PI)*RAD2DEG;

    return vec3(0, 1, 1);//julia(vec2(longi, lati), vec2(1, 1)*360.);
}
...
void main(){
    ...
    Material wood = Material(vec3(1, 1, 1), 1., true, 0.5, false, 1.0);
    ...
    p[2] = Plane(vec3(poff, 0.0, 0.0), vec3(-1.0, 0.0, 0.0), vec3(0.0, 0.0, -1.0), vec3(0.0, 1.0, 0.0), wood);
    p[3] = Plane(vec3(-poff, 0.0, 0.0), vec3(1.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0), vec3(0.0, -1.0, 0.0), wood);
    ...
    s[2] = Sphere(vec3(cx, cy, cz)*sphere_offset*3.0, sphere_radius, glass);

    s[3] = Sphere(vec3(-cx, -cy, -cz)*sphere_offset*4.5, sphere_radius, glass);
